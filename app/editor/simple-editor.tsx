...
// --- Components ---
import { MainToolbarContent, PAGE_SIZES } from "@/components/toolbar"
...
export const SimpleEditor = forwardRef<
  SimpleEditorRef,
  { content?: string }
>(({ content = "# Enter text to continue" }, ref) => {
  ...
  const [pageSize, setPageSize] =
    useState<keyof typeof PAGE_SIZES>("A4")
  ...
  const editor = useEditor({
    ...
    onPaste: async (event, slice) => {
      ...
      if (isMarkdown) {
        ...
        const formattedText =
          await handleMarkdownFormatting(pastedText)
        ...
        if (editor && formattedText) {
          ...
          try {
            ...
            setError(
              `Error parsing markdown: ${err instanceof Error
                ? err.message
                : String(err)
              }`
            )
  ...
  useImperativeHandle(
    ...
    () => ({
      getMarkdown: () => {
         ...
         *
         * getMarkdown() serializes the CURRENT
         * Tiptap document.
         *
  ...
  // ============================================================
  // UPDATE EDITOR WHEN PARENT CONTENT CHANGES
  // ============================================================
